...
void DefaultSceneLayer::_CreateScene()
{
	...
	if (loadScene && std::filesystem::exists("scene.json")) {
		...
		// This will be the reflective material, we'll make the whole thing 90% reflective
		Material::Sptr DashManMaterial = ResourceManager::CreateAsset<Material>(reflectiveShader);
		{
			DashManMaterial->Name = "Dash Man Material";
			DashManMaterial->Set("u_Material.Diffuse", dashmanTex);
			DashManMaterial->Set("u_Material.Shininess", 0.3f);
		}
		...
		{
			camera->SetPostion(glm::vec3(0.f, 0.f, 2.0f));
			camera->SetRotation(glm::vec3(90.f, 0.f, 0.f));
			camera->LookAt(glm::vec3(0.0f, 20.f, 2.f));
		...
		GameObject::Sptr DashMan = scene->CreateGameObject("Dash Man");
		{
			...
			RenderComponent::Sptr renderer = DashMan->Add<RenderComponent>();
			renderer->SetMesh(dashmanMesh);
